...
Cea708Stream.prototype.multiByteCharacter = function (i, service, blockEnd) {
  ...
  // P16 parameters are character data, not independent G0/G1 characters.
  // A truncated command must not consume bytes from the next service block
  // or reinterpret its remaining parameter as text or a command.
  if (i + 2 >= end) {
    return end - 1;
  }

  // A zero high byte carries a single-byte character, including when the
  // service uses an explicit legacy TextDecoder. Do not emit a leading NUL.
  if (packetData[i + 1] === 0) {
    return this.handleText(i + 2, service);
  }

  return this.handleText(i + 1, service, {isMultiByte: true});
};
